...
3. **重置页面**：仅关闭本任务创建且不再需要的标签页，保留用户原有标签页；回到起始 URL，重新加载并确认页面就绪；不要沿用 Explore 阶段的中间状态。
...
"<当前 Skill 目录>/scripts/verify_demo.sh" \
  <实际最终视频> <实际目标宽度> <实际目标高度> <实际目标帧率>
```

验收参数按录制前确认的授权档位设置：Free 为 `1920 1080 30`，Pro 的 2K60 为 `2560 1440 60`，4K60 为 `3840 2160 60`。若目标规格需要 Pro 而当前是 Free，先说明限制，不将降级产物称为 2K/4K。
