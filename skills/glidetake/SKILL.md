...
需要 Chrome 或 Ego Lite、Agent Record 扩展、Node.js/npm 和 FFmpeg。首次使用会自动定位当前项目或已安装版本；找不到时，代理会从 `oil-oil/agent-record` 的对应 GitHub Release 下载并缓存完整桌面伴侣。扩展首次安装和 macOS 屏幕录制授权必须由用户确认，代理不会操作 `chrome://extensions`。
